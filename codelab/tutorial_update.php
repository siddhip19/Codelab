<?php
$conn=mysqli_connect("localhost","root","","codelabdb");
   if($conn==false){
...
$topic_err = $link_err = $description_err = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($topic_err) && empty($link_err) && empty($description_err)){
        ...
        $sql = "UPDATE tutorials SET topic=?, link=?, description=? WHERE id=?";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_topic, $param_link, $param_description, $param_id);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: tutorial_crud.php");
                exit();
            } else{
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($link_err)) ? 'has-error' : ''; ?>">
                            <label>Link</label>
                            <input type="link" name="link" class="form-control" value="<?php echo $link; ?>">
                        ...
                        <a href="tutorial_crud.php" class="btn btn-default">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
