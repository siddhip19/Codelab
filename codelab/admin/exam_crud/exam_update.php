...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($image_err) && empty($type_err) && empty($link_err)){
        // Prepare an update statement
        $sql = "UPDATE cards SET name=?, image=?, type=?, link=? WHERE id=?";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssi", $param_name, $param_image, $param_type, $param_link, $param_id);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($link_err)) ? 'has-error' : ''; ?>">
                            <label>Link</label>
                            <input type="text"  name="link" class="form-control" maxlength="145" value="<?php echo $link; ?>">
                            <span class="help-block"><?php echo $link_err;?></span>
                        </div>
                        <input type="hidden" name="id" value="<?php echo $id; ?>"/>
                        <input type="submit" class="btn btn-primary" value="Submit">
                        <a href="../exam_edit.php" class="btn btn-default">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
